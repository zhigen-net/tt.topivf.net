      ...
      {value ? (
        <div className="space-y-2 rounded-md border p-2">
          {/* 固定高度而不是固定比例：素材横竖都有，按比例缩进这个高度里就不会裁掉内容，
              高度定死了切标签时布局也不跳 */}
          {current
            ? <AssetThumb asset={current} className="h-72 w-full rounded" />
        ...
        <button
          ...
          disabled={disabled}
          className="flex h-72 w-full flex-col items-center justify-center gap-1 rounded-md border border-dashed text-sm text-muted-foreground transition-colors hover:border-primary hover:text-foreground disabled:pointer-events-none disabled:opacity-50"
        >
